...
export class FeeCalculatorService {
  ...
  private calculateReferralFee(category: string, price: number): number {
    ...
    const percentage = parseFloat(structure.referralFeePercentage.toString().replace('%', ''));
  ...
  private calculateWeightHandlingFee(
    ...
    const structure = this.feeStructures
      ...
      .find(f => {
        ...
        return f.weightRange.includes(shippingMode) && 
               f.weightRange.includes(weight <= 12000 ? 'Standard Size' : 'Heavy & Bulky');
      });

    if (!structure) {
      throw new Error(`No weight handling fee structure found for mode: ${shippingMode}`);
    }
  ...
  private calculatePickAndPackFee(productSize: string): number {
    const structures = this.feeStructures
      .filter(f => f.sheetType === 'other' && f.feeType === 'Pick & Pack Fee');
    ...
    if (productSize === 'Standard') {
      ...
      structure = structures.find(f => f.applicableOn === 'Oversize/Heavy & Bulky');
    }
  ...
  private calculateStorageFee(productSize: string, volume: number): number {
    ...
    const ratePerCubicFoot = parseFloat(structure.value.toString().replace('₹', '').split(' ')[0]);
    return ratePerCubicFoot * volume;
  }


  private calculateRemovalFee(productSize: string, shippingSpeed: string): number {
    const structures = this.feeStructures
      .filter(f => f.sheetType === 'other' && f.feeType === 'Removal Fees');

    let structure: FeeStructure | undefined;

    if (productSize === 'Standard') {
      structure = structures.find(f => 
        f.applicableOn === `Standard Size - ${shippingSpeed} Shipping`
      );
    } else {
      structure = structures.find(f => 
        f.applicableOn === `Heavy & Bulky - ${shippingSpeed} Shipping`
      );
    }

    if (!structure || !structure.value) {
      throw new Error(`No removal fee found for size: ${productSize} and shipping speed: ${shippingSpeed}`);
    }
  ...
  async calculateTotalFees(input: CalculatorInput): Promise<CalculatorResponse> {
    ...
    const storageFee = input.volume ? this.calculateStorageFee(input.productSize, input.volume) : 0;

    // Calculate removal fee if shipping speed is provided
    const removalFee = input.shippingSpeed ? 
      this.calculateRemovalFee(input.productSize, input.shippingSpeed) : 0;

    const totalFees = referralFee + weightHandlingFee + closingFee + pickAndPackFee + storageFee + removalFee;
